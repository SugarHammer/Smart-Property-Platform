<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <view v-if="isAuth('examredetails','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('examredetails','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.examredetailsName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('examredetails','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                ...
                <view v-show="item.examquestionName" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">问题名称</text>
                    <text style="width: 70%">{{item.examquestionName}}</text>
                </view>
                <view v-show="item.examquestionTypes" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">问题类型</text>
                    <text style="width: 70%">{{item.examquestionValue}}</text>
                </view>
                <!-- 当前表 -->
                 <view v-show="item.examredetailsUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">问卷编号</text>
                     <text style="width: 70%">
                         <text v-if="item.examredetailsUuidNumber != null">{{item.examredetailsUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.examredetailsMyanswer" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">考生答案</text>
                     <text style="width: 70%">
                         <text v-if="item.examredetailsMyanswer != null">{{item.examredetailsMyanswer}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.examredetailsMyscore" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">问题得分</text>
                     <text style="width: 70%">{{item.examredetailsMyscore}}</text>
                 </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('examredetails','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('examredetails','审核') && item.examredetailsYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
		...
		methods: {
            ...
            async chartDialog() {
                ...
                let res = await _this.$api.requestConditionDataGet("",'newSelectGroupSum',null,{
                    tableName:'examredetails_shouzhi',
                    groupColumn:'examredetails_types',
                    // sumColumn:'examredetails_number',
            ...
            //查询条件切换
            queryChange(e) {
                this.searchForm.examredetailsName = "";
            },
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`examredetails`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('examredetails', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.examredetailsYesnoTypes=shenheTypes;
                            data.examredetailsShenheTime=shijian;
                            await _this.$api.update(`examredetails`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
                ...
                let res = await this.$api.page(`examredetails`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
