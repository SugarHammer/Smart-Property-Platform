<template>
	<div class="main-content">
		...
		<div v-if="showFlag">
			<el-form :inline="true" :model="searchForm" class="form-content">
				...
				<el-row class="ad"
					...
					<el-col :span="zujuanTypes==1?6:24" style="margin-bottom: 10px;">
						...
						<el-tag style="float: right;" size="small">问卷总分:<span style="font-size: 20px;">{{exampapertopicNumber}}</span></el-tag>
					</el-col>
				</el-row>
			</el-form>
			<div class="table-content">
				<el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
					...
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
						prop="exampapertopicNumber" header-align="center" label="问题分数">
						<template slot-scope="scope">
					...
					<el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
						<template slot-scope="scope">
							...
							<el-button v-if="isAuth('exampaper','添加考题')" type="success" icon="el-icon-tickets"
		...
		<!-- 添加问题 -->
		<el-dialog title="添加问题" :visible.sync="questionsTableVisible" width="80%">
				...
				<el-row :gutter="20" class="slt"
					...
					<el-form-item :label="contents.inputTitle == 1 ? '问题名称' : ''">
						<el-input prefix-icon="el-icon-search" v-model="questionsSearchForm.examquestionName"
							placeholder="问题名称" clearable></el-input>
					</el-form-item>
					<el-form-item :label="contents.inputTitle == 1 ? '问题类型' : ''">
						<el-select v-model="questionsSearchForm.examquestionTypes" placeholder="请选择问题类型">
			...
			<el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
				...
				<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="examquestionName"
					header-align="center" label="问题名称">
					<template slot-scope="scope">
					   <span style="display: inline-block;white-space: nowrap; width: 80%;overflow: hidden;text-overflow:ellipsis;">
							{{scope.row.examquestionName}}
						</span>
	...
	export default {
		data() {
			return {
				...
				questionsTableVisible: false, //手动添加问题弹框
				questionsList: [], //问题数据集合
				//问题数据集合分页信息
				...
				json_fields: {
					...
					'问题类型': 'examquestionTypes',
					//本表字段
					'问题分数': "exampapertopicNumber",
				},
		...
		methods: {
			...
			addshiti() {
				if (this.zujuanTypes==1) {
					...
					if(this.danNum>this.danXisting)		{return this.$message.error("单选题数量不能大于现有数量")}
					if(this.duoNum>this.duoXisting)		{return this.$message.error("多选数量数量不能大于现有数量")}
					if(this.panNum>this.panXisting)		{return this.$message.error("判断题数量不能大于现有数量")}
					if(this.tianNum>this.tianXisting)	{return this.$message.error("填空题数量不能大于现有数量")}
