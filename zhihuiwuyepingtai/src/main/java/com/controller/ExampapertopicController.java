...
//                            exampapertopicEntity.setExampaperId(Integer.valueOf(data.get(0)));   //问卷 要改的
//                            exampapertopicEntity.setExamquestionId(Integer.valueOf(data.get(0)));   //问题 要改的
//                            exampapertopicEntity.setExampapertopicNumber(Integer.valueOf(data.get(0)));   //问题分数 要改的
        ...
        ExampapertopicEntity exampapertopic = exampapertopicService.selectById(id);
            if(exampapertopic !=null){
                ...
                //级联表
                    ExampaperEntity exampaper = exampaperService.selectById(exampapertopic.getExampaperId());
                if(exampaper != null){
                    BeanUtils.copyProperties( exampaper , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setExampaperId(exampaper.getId());
                }
                //级联表
                    ExamquestionEntity examquestion = examquestionService.selectById(exampapertopic.getExamquestionId());
                if(examquestion != null){
                    BeanUtils.copyProperties( examquestion , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "username", "password", "newMoney"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setExamquestionId(examquestion.getId());
                }
                //修改对应字典表字段
                dictionaryService.dictionaryConvert(view, request);
...
//            .notIn("exampapertopic_types", new Integer[]{102})
    ...
    */
    ...
    public R submitQuestions(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        examrecordEntity.setExampaperId(exampaperId);//问卷id
        examrecordEntity.setYonghuId(yonghuId);//问卷用户
        ...
        if(examrewrongquestionList.size() > 0){
