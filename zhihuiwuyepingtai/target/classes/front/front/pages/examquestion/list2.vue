	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('examquestion','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
	<view style="">
            ...
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                 ...
                 <view v-show="item.examquestionOptions" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     ...
                     <text style="width: 70%">{{item.examquestionOptions}}</text>
                 </view>
                 <view v-show="item.examquestionAnswer" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     ...
                     <text style="width: 70%">{{item.examquestionAnswer}}</text>
                 </view>
                 <view v-show="item.examquestionAnalysis" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     ...
                     <text style="width: 70%">{{item.examquestionAnalysis}}</text>
                 </view>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 科目 字段 字典表查询方法
		  	let kemuTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'kemu_types'
		  	});
		  	this.kemuTypesList = kemuTypes.data.list;
            ...
            this.categoryList = examquestionTypes.data.list
				...
				let params = {
                    ...
                    yonghuId:this.userId,//查询自己的
							...
							if (this.categoryName != 0) {
                                params.examquestionTypes = this.categoryName
                            }
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`examquestion`, {
                                id:id,
                            });
                            //await _this.$api.requestConditionDataGet('examquestion','zhifu',null,{
                            //    id:id
                            //});
